...
<div class="orderby">
  ...
  {#if data._links?.orderByFirstName}
    <a href="?url={encodeURIComponent(data._links.orderByFirstName.href)}"
      >First name</a
    > |
  {/if}
  {#if data._links?.orderByLastName}
    <a href="?url={encodeURIComponent(data._links.orderByLastName.href)}"
      >Last name</a
    > |
  {/if}
...
{#if data._links?.create}
  <div class="create">
    <a href="contacts?createUrl={encodeURIComponent(data._links.create.href)}"
...
<table>
  {#each data.resource.list as contact}
    <tr>
      <td
        ><a href="contacts?url={encodeURIComponent(contact._links.self.href)}"
          >Edit</a
        ></td
      >
